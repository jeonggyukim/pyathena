...
class H2:
    ...
    def read_H2eq_all(
        self, nums=None, prefix="H2eq_all", savdir=None, force_override=False
    ):
        rr = dict()
        if nums is None:
            nums = self.nums
    ...
    def read_H2eq(self, num, prefix="H2eq", savdir=None, force_override=False):
        ...
        dd = ds.get_field(["nH", "xH2", "xHII", "rad_energy_density_LW_diss", "xi_CR"])
        ...
        try:
            ...
            xi_CR0 = (
                h["xi_CR0"]
                .loc[(h["time_code"] - ds.domain["time"]).abs().idxmin()]
                .iloc[0]
            )
        except AttributeError:
            xi_CR0 = h["xi_CR0"].loc[
                (h["time_code"] - ds.domain["time"]).abs().idxmin()
            ]

        xi_CR = dd["xi_CR"].data.flatten()
        zeta_LW = (
            dd["rad_energy_density_LW_diss"] * u.energy_density.value
        ).data.flatten()
        nH = dd["nH"].data.flatten()
        nH2 = (dd["nH"] * dd["xH2"]).data.flatten()
        xH2eq_constCR = calc_xH2eq(
            nH, xi_CR0, k_gr=par["cooling"]["kgr_H2"], zeta_LW=zeta_LW
        )
        xH2eq_noLW = calc_xH2eq(nH, xi_CR, k_gr=par["cooling"]["kgr_H2"], zeta_LW=0.0)
        xH2eq_noLW_constCR = calc_xH2eq(
            nH, xi_CR0, k_gr=par["cooling"]["kgr_H2"], zeta_LW=0
        )
        ...
        k = "xH2eq-xH2"
        hw, binex, biney = np.histogram2d(
            2.0 * xH2eq,
            2.0 * (dd["xH2"]).data.flatten(),
