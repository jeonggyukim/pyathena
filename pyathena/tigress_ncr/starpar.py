...
class StarPar:
    ...
    def read_starpar_all(self, prefix="starpar_all", savdir=None, force_override=False):
        rr = dict()
    ...
    def read_starpar(self, num, savdir=None, force_override=False):
        ...
        isrc = np.logical_and(sp["mage"] < agemax, sp["mass"] != 0.0)
        ...
        r["stdz_mass"] = np.sqrt(
            np.average((sp["x3"] - r["z_mean_mass"]) ** 2, weights=sp["mass"])
        )
        r["stdz_Qi"] = np.sqrt(
            np.average((sp["x3"] - r["z_mean_Qi"]) ** 2, weights=sp["Qi"])
        )
        r["stdz_LFUV"] = np.sqrt(
            np.average((sp["x3"] - r["z_mean_LFUV"]) ** 2, weights=sp["L_FUV"])
        )
    ...
    def get_Jrad_pp_all(self, savdir=None, force_override=False):
        ...
        for i in self.nums_starpar:
            ...
            PE_pp.append(
                xr.DataArray(Jrad["PE"], coords=[zpc], dims=["z"]).assign_coords(
                    time=self.time
                )
            )
            LW_pp.append(
                xr.DataArray(Jrad["LW"], coords=[zpc], dims=["z"]).assign_coords(
                    time=self.time
                )
            )
        dset = xr.Dataset()
