...
def plot_projection_Z(surfname, starfname, stars=True, writefile=True, runaway=True, norm_factor=2., aux={}):
    ...
    if 'cmap' in aux:
        im1.set_cmap(aux['cmap'])
    if 'clim' in aux:
        im1.set_clim(aux['clim'])
    ax1.text(extent[0] * 0.9, extent[3] * 0.9,
             't=%3d Myr' % tMyr, ha='left', va='top', **(texteffect()))
    if stars:
        scatter_sp(sp, ax1, axis='y', runaway=runaway, type='surf', norm_factor=norm_factor)
    ...
    if 'cmap' in aux:
        im2.set_cmap(aux['cmap'])
    if 'clim' in aux:
        im2.set_clim(aux['clim'])
    if stars:
        scatter_sp(sp, ax2, axis='x', runaway=runaway, type='surf')
