...
def do_surfmaps(s,num):
    ...
    fields=['density', 'xHI', 'xH2', 'xHII', 'xe', 'nesq',
            'specific_scalar[0]', 'specific_scalar[1]']
    ...
    for f in prjdata:
        ...
        if f == 'EM':
            kwargs = dict(norm = LogNorm(EM0*1.e-4,EM0*100),cmap = plt.cm.plasma)
        elif f.startswith('ne'):
            kwargs = dict(norm = LogNorm(n0*1.e-4,n0*100),cmap = cmr.rainforest)
        else:
