...
class Snapshot_HIH2EM:
    def plt_snapshot_HIH2EM(self, num):
        ...
        fig, axes = plt.subplots(
            nr,
            nc,
            figsize=(16, 21),  # constrained_layout=True,
            gridspec_kw=dict(
                hspace=0.1,
                wspace=0.1,
        ...
        for i, label in enumerate(labels):
            ...
            if i < 3:
                cb = plt.colorbar(
                    plt.cm.ScalarMappable(norm=norm, cmap=plt.cm.pink_r),
                    cax=cax,
                ...
                cb = plt.colorbar(
                    plt.cm.ScalarMappable(norm=norm_EM, cmap=plt.cm.plasma),
                    cax=cax,
                    orientation="horizontal",
                    label=label,
                )
        ...
        savname = osp.join(
            savdir, "{0:s}_HIH2EM_{1:04d}.png".format(self.basename, num)
        )
