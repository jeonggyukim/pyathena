...
class LoadSimTIGRESSPP(LoadSim,Hst,SliceProj,Fields,Timing):
    ...
    def __init__(self, basedir, savdir=None, load_method='pyathena', verbose=False):
        ...
        super(LoadSimTIGRESSPP,self).__init__(basedir, savdir=savdir,
        ...
        if self.par["cooling"]["coolftn"] == "tigress":
            ...
            logLam=interp1d(cooltbl["logT1"],np.log10(cooltbl["Lambda"]),fill_value="extrapolate")
            logGam=interp1d(cooltbl["logT1"],np.log10(cooltbl["Gamma"]),fill_value="extrapolate")
...
class LoadSimTIGRESSPPAll(object):
    ...
    def __init__(self, models=None, muH=None):
        ...
        for mdl, basedir in models.items():
            if not osp.exists(basedir):
                print('[LoadSimTIGRESSPPAll]: Model {0:s} doesn\'t exist: {1:s}'.format(
                    mdl,basedir))
            else:
    ...
    def set_model(self, model, savdir=None, load_method='pyathena', verbose=False):
        ...
        try:
            ...
            self.sim = LoadSimTIGRESSPP(self.basedirs[model], savdir=savdir,
                                        load_method=load_method, verbose=verbose)
