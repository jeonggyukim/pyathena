        ...
        if setgrid:
            if ds == None:
                ...
                if ds.grids[0]['filename'] != self.flist[0]:
                    for g, f in zip(ds.grids, self.flist):
                        g['filename'] = f
                self.grids = ds.grids
...
class AthenaZprof(object):
    def __init__(self, filename, stitch=True, clean=False, forced_clean=False):
        ...
        if clean:
            for p in self.plist:
                self._clean(p, forced=forced_clean)
    ...
    def _stitch(self, phase):
        ...
        if self.mpi:
            flist += glob.glob(os.path.join(self.path, 'id*/%s-id*.%s.%s.%s' % (self.id, self.step, phase, self.ext)))
        nf = len(flist)
...
class AthenaDataSet(AthenaDomain):
    def _setup(self):
        ...
        if ('rad_energy_density0' in self.field_list and
           'rad_energy_density1' in self.field_list
            and 'rad_energy_density2' in self.field_list) or \
           ('rad_energy_density0' in self.field_list
            and 'rad_energy_density1' in self.field_list
                and not 'rad_energy_density_LW' in self.field_list):
    ...
    def _read_grid_data(self, grid, field):
        ...
        grid['data'][field] = var
        if nvar == 3:
            self._set_vector_field(grid, field)
    ...
    def _get_derived_field(self, grid, field):
        ...
        if field in gd:
            ...
            v2 = gd['velocity2']
            v3 = gd['velocity3']
            if field == 'kinetic_energy1':
                return 0.5 * den * v1**2
            if field == 'kinetic_energy2':
                return 0.5 * den * v2**2
            if field == 'kinetic_energy3':
                return 0.5 * den * v3**2
            ...
            if field == 'magnetic_energy1':
                return 0.5 * B1**2
            if field == 'magnetic_energy2':
                return 0.5 * B2**2
            if field == 'magnetic_energy3':
                return 0.5 * B3**2
            ...
            if field == 'plasma_beta':
                return press * 2.0 / (B1**2 + B2**2 + B3**2)
            ...
            B2 = gd[vfield + '2']
            B3 = gd[vfield + '3']
            if field == 'alfven_velocity1':
                return B1 / np.sqrt(den)
            if field == 'alfven_velocity2':
                return B2 / np.sqrt(den)
            if field == 'alfven_velocity3':
                return B3 / np.sqrt(den)
            ...
            if not 'xn' in self.derived_field_list:
                # With Chang-Goo's cooling fn.
                self._read_grid_data(grid, 'density')
                self._read_grid_data(grid, 'pressure')
            ...
            B2 = gd[vfield + '2']
            B3 = gd[vfield + '3']
            if field == 'magnetic_stress1':
                return B2 * B3
            if field == 'magnetic_stress2':
                return B1 * B3
            if field == 'magnetic_stress3':
                return B1 * B2
            return B1 * B2
            ...
            v2 = gd['velocity2']
            v3 = gd['velocity3']
            if field == 'reynold_stress1':
                return den * v2 * v3
            if field == 'reynold_stress2':
                return den * v1 * v3
            if field == 'reynold_stress3':
                return den * v1 * v2
            return den * v1 * v2
            ...
            Gcode = u['Gcode']
            if field == 'gravity_stress1':
                return g2 * g3 / 4 / np.pi / Gcode
            if field == 'gravity_stress2':
                return g1 * g3 / 4 / np.pi / Gcode
            if field == 'gravity_stress3':
    ...
    def _get_slab_grid(self, slab=1, verbose=False):
        ...
        if verbose:
            print(("XY slab from z=%g to z=%g" % (grids[0]['left_edge'][2], grids[0]['right_edge'][2])))
        return grids
...
class AthenaSlice(AthenaRegion):
    ...
    def slice(self, ds, *args, **kwargs):
        if 'axis' in kwargs:
            ...
            print('need to specify symmetric axis')

        if 'center' in kwargs:
            c = kwargs['center']
        else:
            print('need to specify center of the plane')

        if 'field' in kwargs:
            field = kwargs['field']
        else:
            print('need to specify field')
...
class AthenaSurf(object):
    def __init__(self, ds, *args, **kwargs):
        ...
        if 'field' in kwargs:
            ...
            field = 'density'
        if 'weighted_field' in kwargs:
            wfield = kwargs['weighted_field']
        else:
            wfield = None
