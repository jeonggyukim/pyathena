...
class SliceProj:
    ...
    def plt_snapshot(
        self,
        num,
        fields=["Sigma_gas", "T", "pok", "nH", "Bmag", "rSN"],
        xwidth=2,
        norm_factor=5.0,
        agemax=40.0,
        agemax_sn=40.0,
        ...
        axlabels = set({"x","y","z"})
        for g_, ax_ in zip([g1,g2,g3],["x","y","z"]):
            extent = dat["prj"]["extent"][ax_]
            jshift = 0

            for i, (ax, f) in enumerate(zip(g_, fields)):
                ...
                if i == (nxf // 2 - 1):
                    labels = labels = sorted(list(axlabels - set({ax_})))
                    ax.set(xlabel=f"{labels[0]} [pc]", ylabel=f"{labels[1]} [pc]")
                else:
