...
def create_all_pickles_mhd(force_recal=False, force_redraw=False, verbose=True, **kwargs):
    ...
    if not os.path.isdir(dir + 'slice/'):
        os.mkdir(dir + 'slice/')
    if not os.path.isdir(dir + 'surf/'):
        os.mkdir(dir + 'surf/')
    ...
    for i, f in enumerate(fname):
        ...
        if do_task:
            ...
            if tasks['surf']:
                create_projection(ds, surfname, conversion={'z': ds.domain['Lx'][2] * to_surf})
            if tasks['slice']:
                create_slices(ds, slcfname, slc_fields, factors=mul_factors, force_recal=force_recal)
