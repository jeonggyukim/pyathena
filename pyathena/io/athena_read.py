...
def athdf(filename, raw=False, data=None, quantities=None, dtype=None, level=None,
    ...
    """
    Read .athdf files generated by Athena++ and return a dictionary of data arrays.

    This function reads HDF5 files produced by Athena++ simulations and extracts
    simulation data, including grid coordinates, physical quantities, and metadata.
    It supports various options for data selection, subsampling, and refinement level
    adjustments.

    Parameters:
    ----------
    filename : str
        Path to the .athdf file to be read.
    raw : bool, optional
    ...
    -------
    dict
        A dictionary containing the extracted data arrays. Keys include:
        - 'x1f', 'x2f', 'x3f': Face coordinates for each dimension.
        - 'x1v', 'x2v', 'x3v': Cell center coordinates for each dimension.
        - Quantities specified in the `quantities` parameter or all quantities in the file.
        - Metadata attributes from the file.

    Raises:
    ------
    AthenaError
        If the file is not formatted as expected or if invalid parameters are provided.

    Notes:
    -----
    - This function supports Cartesian, cylindrical, and spherical coordinate systems.
    - Subsampling and fast restriction are incompatible with ghost zones.
    - If `raw` is True, the function skips merging MeshBlocks and returns data as-is.
    - For advanced use cases, custom volume and coordinate functions can be provided.

    Examples:
    --------
    # Read all data from a file
    data = athdf('simulation.athdf')

    # Read specific quantities at a specific refinement level
    data = athdf('simulation.athdf', quantities=['density', 'velocity'], level=2)

    # Read data with spatial bounds
    data = athdf('simulation.athdf', x1_min=0.0, x1_max=1.0, x2_min=0.0, x2_max=1.0)

    # Use custom volume function
    def custom_vol_func(xm, xp, ym, yp, zm, zp):
        return (xp - xm) * (yp - ym) * (zp - zm)
    data = athdf('simulation.athdf', vol_func=custom_vol_func)
    """
