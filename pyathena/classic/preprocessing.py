    ...
    for ext in ['zprof', 'hst', 'rst', 'starpar']:
        ...
        if ext is 'rst':
            for i in range(1, nprocs):
                ...
                if len(files):
                    for f in files:
                        shutil.move(f, f.replace('id{}/'.format(i), 'rst/'))
...
def zprof_to_xarray(base, problem_dir, problem_id):
    ...
    if not os.path.isdir(zpmerge_dir):
        os.mkdir(zpmerge_dir)
...
def processing_history_dump(hst, params, hstfile):
    ...
    for ax in ['1', '2', '3']:
        ...
        if mhd:
            h['vA{}'.format(ax)] = np.sqrt(2 * hst['x{}ME'.format(ax)] / hst['mass'])
...
def processing_zprof_dump(h, rates, params, zprof_ds, hstfile):
    ...
    mhd = False
    if 'B1' in zprof_ds.fields.to_index():
        mhd = True
    nscal = 0
...
def draw_history(h_zp, metadata, figfname=''):
    ...
    for f, ylabel, yscale, subfields, ybottom, ytop in metadata:
        ...
        if f in h_zp:
            ...
            if len(subfields) > 0:
                if f in labels:
                    l.set_label(labels[f])
                for sf in subfields:
                    if sf in h_zp:
                        l, = plt.plot(h_zp['tMyr'], h_zp[sf])
                        if f in labels:
                            l.set_label(labels[sf])
...
def doall(base, problem_id, problem_dir=None, do_pickling=True, use_yt=True):
    ...
    for k in done:
        if done[k]:
            print('{}.{} is moved'.format(problem_id, k))
    ...
    if os.path.isfile(zpfile):
        recal_history(base, problem_dir, problem_id)
