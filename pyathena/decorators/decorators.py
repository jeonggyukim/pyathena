...
def check_netcdf(func):
    ...
    def wrapper(cls, *args, **kwargs):
        ...
        if not force_override and osp.exists(fnetcdf):
            if read_all:
                cls.logger.info(
                    'Read {0:s} from existing NetCDF dump.'.format(prefix) +\
                    ' Set force_override to True if time range should be updated.')

            ds = xr.open_dataset(fnetcdf)
            return ds
        else:
            cls.logger.info('Construct directly without reading from NetCDF')
            ds = func(cls, **kwargs)
            ...
            try:
                ...
                cls.logger.warning('Could not netcdf to {0:s}.'\
                                   .format(fnetcdf))
