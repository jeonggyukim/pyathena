...
def J_over_JUV_inside_slab(tau, tau_SF):
    ...
    return (
        0.5 / tau_SF * (2.0 - expn(2, 0.5 * tau_SF - tau) - expn(2, 0.5 * tau_SF + tau))
    )
...
def read_rad_lost(filename, force_override=False, verbose=False):
    ...
    if (
        not force_override
        and os.path.exists(fpkl)
        and os.path.getmtime(fpkl) > os.path.getmtime(filename)
    ):
    ...
    if verbose:
        print(
            "[read_radiators]: pickle does not exist or file updated."
...
def calc_Jrad_pp(s, num):
    ...
    for f in ("LW", "PE"):
        sigmad[f] = (
            par["opacity"]["sigma_dust_{0:s}0".format(f)] * par["problem"]["Z_dust"]
        )
