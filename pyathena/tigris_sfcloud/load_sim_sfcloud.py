...
class LoadSimSFCLOUD(LoadSim, Hst, SliceProj, Fields, Timing):
    ...
    def __init__(self, basedir, savdir=None, load_method="pyathena", verbose=False):
        ...
        super(LoadSimSFCLOUD, self).__init__(
    ...
    def __init__(self, models=None, muH=None):
        ...
        for mdl, basedir in models.items():
            if not osp.exists(basedir):
                print(
                    "[LoadSimSFCLOUDAll]: Model {0:s} doesn't exist: {1:s}".format(
                        mdl, basedir
                    )
                )
            else:
