...
class CIFineStructureCooling(CoolingChannel):
    ...
    def evaluate(
        ...
        if d_out is not None:
            ...
            # chain rule runs through the piecewise electron-strength
            # polynomials and the steady-state 3-level solve; FD
            # bootstrap matches the substep-damping accuracy in one
            # extra _compute_lambda call. The T = 1000 K piecewise
            # boundary gives an indeterminate delta for cells whose
            # T and T * (1 + dT_rel) straddle it, same as the other
            # gated bootstrap channels.
            _DT_REL = 1.0e-3
            T_orig = state.get_scratch('cooling:ci:T_orig')
            out_tp = state.get_scratch('cooling:ci:out_tp')
