...
class RestartHandler(object):
    ...
    def remove_all_targets(self):
        ...
        self.ideg = 0
        self.iref = 0
        if hasattr(self, 'new_x3min'):
            delattr(self, 'new_x3min')
        if hasattr(self, 'new_x3max'):
            delattr(self, 'new_x3max')
    ...
    def write(self, outdir=None, pid="newrst", itime=0):
        ...
        if self.verbose:
            print("new restart dump is written in: {}".format(new_fname))
        return new_fname
...
def _write_onefile(newfile, data_part, data_par, stars):
    ...
    for block in ['par', 'time']:
        fp.write(data_par[block])
...
def _to_eint(rstdata, neg_correct=True):
    ...
    if '1-FIELD' in rstdata:
        for i, f in enumerate(['1-FIELD', '2-FIELD', '3-FIELD']):
            if f == '1-FIELD':
                Bc = 0.5 * (rstdata[f][:, :, :-1] + rstdata[f][:, :, 1:])
            elif f == '2-FIELD':
                Bc = 0.5 * (rstdata[f][:, :-1, :] + rstdata[f][:, 1:, :])
            elif f == '3-FIELD':
                Bc = 0.5 * (rstdata[f][:-1, :, :] + rstdata[f][1:, :, :])
    ...
    if neg_correct:
        ...
        if len(eint[eint < 0]) > 0:
            sys.exit("negative energy persist!")
...
def _refine(rstdata, scalar=0):
    ...
    cc_varnames = ['DENSITY', '1-MOMENTUM', '2-MOMENTUM', '3-MOMENTUM',
                   'ENERGY']
    if 'POTENTIAL' in rstdata:
        cc_varnames += ['POTENTIAL']
    if '1-FIELD' in rstdata:
        fc_varnames = ['1-FIELD', '2-FIELD', '3-FIELD']
    else:
        fc_varnames = []
...
def _parse_par(rstfile):
    ...
    while 1:
        ...
        if block in ['problem', 'domain1', 'time']:
            sp = line.strip().split()
            if len(sp) >= 3:
                par[block][sp[0]] = eval(sp[2])
        else:
            sp = line.split('=')
            if len(sp) == 2:
                par[block][sp[0].strip()] = sp[1].split('#')[0].strip()
...
def _read_one_grid(rstfile, verbose=False, starghost=True):
    ...
    while 1:
        ...
        if _parse_rst(var, par, rst):
            ...
            if vtype == 'param':
                if verbose:
                    print(var, struct.unpack('<' + ndata * dtype, data))
            elif vtype == 'star':
