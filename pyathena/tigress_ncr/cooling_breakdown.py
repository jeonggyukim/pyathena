...
def draw_jointpdfs(s, num, pdf=None, zrange=None, save=True, xHI=False):
    ...
    if pdf is not None:
        ...
        pdf_cool = (
            pdf[list(coolkey) + ["total_cooling"]]
            .sum(dim="time")
            .rename(total_cooling="total")
        )
        pdf_heat = (
            pdf[list(heatkey) + ["total_heating"]]
            .sum(dim="time")
            .rename(total_heating="total")
        )
    ...
    for c in list(coolkey):
        ...
        plt.title("")
        ax.annotate(
            r"$\mathcal{{L}}_{{\rm {}}}$".format(c.replace("_", "-")),
            (0.95, 0.95),
    ...
    for h in list(heatkey):
        ...
        plt.title("")
        ax.annotate(
            r"$\mathcal{{G}}_{{\rm {}}}$".format(h.replace("_", "-")),
            (0.95, 0.95),
            xycoords="axes fraction",
            fontsize="x-small",
            ha="right",
    ...
    netcool = (
        pdf_cool["total"] * pdf_cool.attrs["total_cooling"]
        - pdf_heat["total"] * pdf_heat.attrs["total_heating"]
    )
    ...
    netcool.plot(
        ...
        cmap=cmap_cool,
        cbar_kwargs=dict(
            label=r"$\frac{d^2(\mathcal{L}-\mathcal{G})/\mathcal{L}_{\rm tot}}" + xl2
        ),
    )
    (-netcool).plot(norm=LogNorm(1.0e-5, 10), ax=ax, cmap=cmap_heat, add_colorbar=False)
    plt.title("")
    plt.ylim(1, 8)
    plt.ylabel(r"$\log\,T\,[{\rm K}]$")
    if xHI:
    ...
    if save:
        if pdf is None:
            plt.savefig(
                os.path.join(
                    savdir, "{}.{:04d}.coolheat.jointpdf.png".format(s.problem_id, num)
                )
            )
        else:
            plt.savefig(
                os.path.join(savdir, "{}.coolheat.jointpdf.png".format(s.problem_id))
            )
...
def draw_Tpdf(s, num, pdf=None, zrange=None, save=True):
    ...
    if not s.iCoolH2colldiss:
        exclude_cool = exclude_cool | {"H2_colldiss"}
    if not s.iCoolH2rovib:
        exclude_cool = exclude_cool | {"H2_rovib"}
    if not s.iCoolHIcollion:
        exclude_cool = exclude_cool | {"HI_collion"}
    ...
    plt.sca(axes[5])
    plt.step(
        x, pdf_cool_T["total"], label="total cooling", where="mid", color="tab:blue"
    )
    ...
    if save:
        if pdf is None:
            plt.savefig(
                os.path.join(
                    savdir, "{}.{:04d}.coolheat.Tpdf.png".format(s.problem_id, num)
                )
            )
        else:
            plt.savefig(
                os.path.join(savdir, "{}.coolheat.Tpdf.png".format(s.problem_id))
            )

    return fig


def draw_sorted_contribution(
    s, pdf, num, T1=6.0e3, T2=1.2e4, phname="warm", thresh=1.0e-2, save=True
):
    ...
    coolset_sorted = np.array(coolset)[
        warm[coolset].to_array().mean(dim="time").argsort().data[::-1]
    ]
    heatset_sorted = np.array(heatset)[
        warm[heatset].to_array().mean(dim="time").argsort().data[::-1]
    ]
    ...
    plt.sca(axes[1])
    plt.boxplot(
        (warm[coolset_sorted] / warm["cool"]).to_array().T, labels=coolset_sorted
    )
    plt.yscale("log")
    ...
    plt.sca(axes[3])
    plt.boxplot(
        (warm[heatset_sorted] / warm["cool"]).to_array().T, labels=heatset_sorted
    )
    plt.yscale("log")
