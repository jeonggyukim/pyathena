...
class PDF:
    ...
    def read_pdf2d(
        ...
        weight_fields_def = ["nH", "2nH2", "nHI", "nHII", "nH", "2nH2", "nHI", "nHII"]
        ...
        fields = np.unique(
            np.append(
                np.unique(bin_fields), np.unique(weight_fields + ["xHI", "xH2", "xHII"])
            )
        )
    ...
    def plt_pdf2d(
        ...
        if wfield is not None:
            hist = wfield
            ax.annotate(
                wfield, (0.05, 0.95), xycoords="axes fraction", ha="left", va="top"
            )
        ...
        try:
            ...
            ax.pcolormesh(
                dat[bf]["xe"], dat[bf]["ye"], pdf, norm=norm, cmap=cmap, **kwargs
            )
            ...
            ax.set(
                xscale=xscale,
                yscale=yscale,
                xlabel=self.dfi[kx]["label"],
                ylabel=self.dfi[ky]["label"],
            )
        except KeyError:
            pass
    ...
    def plt_pdf2d_all(
        ...
        if plt_zprof:
            zpa = s.read_zprof(
                ["whole", "2p", "h"], savdir=savdir, force_override=force_override
            )
        ...
        ax.set(
            xlim=(ds.domain["le"][0], ds.domain["re"][0]),
        ...
        if plt_zprof:
            ...
            for ph, color in zip(("whole", "2p", "h"), ("grey", "b", "r")):
                ...
                ax.set(
                    xlabel="z [kpc]",
                    ylabel=r"$\langle n_{\rm H}\rangle\;[{\rm cm}^{-3}]$",
                    ylim=(1e-5, 5e0),
                )
                ax.legend(loc=1)
        ...
        fig.suptitle(
            suptitle + " t=" + str(int(ds.domain["time"])),
            x=0.5,
            y=1.02,
            va="center",
            ha="center",
        ...
        if savefig:
            ...
            savname = osp.join(
                savdir, "{0:s}_{1:04d}_pdf2d.png".format(self.basename, num)
            )
    ...
    def load_one_jointpdf(
        ...
        """
        zmin, zmax = zrange
        savdir = "{}/jointpdf_z{:02d}-{:02d}/{}-{}-{}".format(
            self.savdir, int(zmin / 100), int(zmax / 100), xf, yf, wf
        )
        ...
        if (
            not force_override
            and osp.exists(fpdf)
            and osp.getmtime(fpdf) > osp.getmtime(self.fvtk)
        ):
            self.logger.info(
                "[jointpdf]: Reading Joint PDFs of {} and {} weigthed by {} at z in +-({},{})".format(
                    xf, yf, wf, zmin, zmax
                )
            )
    ...
    def one_jointpdf(self, ds, xf, yf, wf, zmin=0, zmax=300):
        ...
        data_zcut = xr.concat(
            [data.sel(z=slice(-zmax, -zmin)), data.sel(z=slice(zmin, zmax))], dim="z"
        )
