...
def grid_msp(s, num, agemin, agemax):
    ...
    if len(sp.columns) > 0:
        ...
        if 'mage' in sp:
            ...
            sp = sp[(sp.age*s.u.Myr < agemax)&
                    (sp.age*s.u.Myr >= agemin)]
...
def get_weights(s,dat,direction='upper'):
    ...
    dat2d=xr.Dataset()
    if direction == 'upper':
        dat2d['wself'] = -(dat['weight_self']*dz).sel(z=slice(0, s.domain['re'][2])).sum(dim='z')
        dat2d['wext'] = -(dat['weight_ext']*dz).sel(z=slice(0, s.domain['re'][2])).sum(dim='z')
    elif direction == 'lower':
        dat2d['wself'] = (dat['weight_self']*dz).sel(z=slice(s.domain['le'][2],0)).sum(dim='z')
        dat2d['wext'] = (dat['weight_ext']*dz).sel(z=slice(s.domain['le'][2],0)).sum(dim='z')
    dat2d['wtot'] = dat2d['wself']+dat2d['wext']
    return dat2d*s.u.pok

def get_pressures(s,dat,dz=None):
    if dz is None: dz = s.domain['dx'][2]
    dat2d=xr.Dataset()
    slc = dat.sel(z=slice(-dz,dz))
    twop = slc['T']<2.e4
    Pth = slc['pressure']
    Pturb = slc['density']*slc['velocity3']**2
    Pimag = 0.5*(slc['cell_centered_B1']**2 + slc['cell_centered_B2']**2 - slc['cell_centered_B3']**2)
...
def get_sfrmap(s,num,tbin=40):
    ...
    return sfr.to_dataset(name=f'sigma_sfr')
