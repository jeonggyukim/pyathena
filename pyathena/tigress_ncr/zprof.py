...
class Zprof(ReadZprofBase):
    def read_zprof_new(self, phase="all", savdir=None, force_override=False):
        ...
        if phase == "all":
            phase = ["phase{}".format(i) for i in range(1, nphase)]
        else:
    ...
    def _read_zprof(self, phase="whole", savdir=None, force_override=False):
        ...
        try:
            ds["Jrad_LW"] = (
                ds["Erad1"] * u.energy_density * ac.c.cgs.value / (4.0 * np.pi)
            )
            ds["Jrad_PE"] = (
    ...
    def load_zprof_for_wind(self, fromfile=True, tofile=True):
        ...
        if fromfile:
            try:
                with xr.open_dataset(
                    os.path.join(self.savdir, "zprof", "zprof_out.nc")
                ) as zp_out:
                    self.logger.info("zprof_out.nc is loaded")
                    self.zp_outflux = zp_out
                with xr.open_dataset(
                    os.path.join(self.savdir, "zprof", "zprof_net.nc")
                ) as zp_net:
        ...
        pflux["mom_kin"] = zp["pFzM3"] * u.momentum_flux
        try:
            pflux["mom_th"] = zp["pP"] * u.momentum_flux
        except KeyError:
            pflux["mom_th"] = zp["P"] * u.momentum_flux
        pflux["mom"] = pflux["mom_kin"] + pflux["mom_th"]
        if self.test_spiralarm():
            # spiral arm models
            Om = self.par["problem"]["Omega"]
            R0 = self.par["problem"]["R0"]
            Lx = self.domain["Lx"][0]
            pattern = self.par["problem"]["pattern"]
            marm = self.par["problem"]["marm"]
            vcirc = Om * R0 * (1 - pattern)
            sini = marm * Lx / (2 * np.pi * R0)
            vy0 = vcirc
            vx0 = vcirc * sini
            print(vx0, vy0)
            ...
            pflux["energy_kin"] = (
                zp["pFzE1"] + zp["pFzE2"] + zp["pFzE3"]
            ) * u.energy_flux
        pflux["energy_th"] = zp["pFzP"] * u.energy_flux
        pflux["energy"] = pflux["energy_kin"] + pflux["energy_th"]
        pflux["A"] = zp["pA"]
        pflux["d"] = zp["pd"]

        nflux = xr.Dataset()
        nflux["mass"] = zp["mFzd"] * u.mass_flux
        nflux["mom_kin"] = zp["mFzM3"] * u.momentum_flux
        try:
            nflux["mom_th"] = zp["mP"] * u.momentum_flux
        except KeyError:
            nflux["mom_th"] = zp["P"] * u.momentum_flux
        nflux["mom"] = nflux["mom_kin"] + nflux["mom_th"]
        nflux["energy_kin"] = (zp["mFzE1"] + zp["mFzE2"] + zp["mFzE3"]) * u.energy_flux
        if self.test_spiralarm():
            # <0.5*rho v_z v_x^2> = <0.5*rho v_z v_x,t^2>
            #                     - <rho v_z v_x>v_x0 - 0.5<rho v_z> v_x0**2
            ...
            nflux["energy_kin"] = (
                zp["mFzE1"] + zp["mFzE2"] + zp["mFzE3"]
            ) * u.energy_flux
        nflux["energy_th"] = zp["mFzP"] * u.energy_flux
        nflux["energy"] = nflux["energy_kin"] + nflux["energy_th"]
        nflux["A"] = zp["mA"]
        nflux["d"] = zp["md"]
        ...
        nflux.coords["z"] = -nflux.z
        nflux.coords["z_kpc"] = nflux.z / 1.0e3
        nflux["mass"] *= -1
        nflux["energy"] *= -1
        nflux["energy_kin"] *= -1
        nflux["energy_th"] *= -1
    ...
    def load_zprof_for_phase(self, thermal_only=False, fromfile=True, tofile=True):
        if fromfile:
            try:
                with xr.open_dataset(
                    os.path.join(self.savdir, "zprof", "zprof_th.nc")
                ) as zp_th:
                    self.logger.info("zprof_th.nc is loaded")
                    self.zp_th = zp_th
                if not thermal_only:
                    with xr.open_dataset(
                        os.path.join(self.savdir, "zprof", "zprof_ch.nc")
                    ) as zp_ch:
        ...
        if not thermal_only:
            zpch = self.read_zprof(
                phase=["mol", "CNM", "UNM", "WNM", "pi", "h1", "h2", "whole"]
            )
...
def plt_zprof_avg_compare(
    sa,
    models=None,
    phase=["whole", "c", "u", "w", "h1", "h2"],
    ls=["-", "--", ":", "-."],
    field="d",
    xlim=dict(whole=None, w=None, h1=None, h2=None, c=(-500, 500), u=(-500, 500)),
    ylim=dict(
        d=dict(
            whole=(1e-4, 1e1),
            c=(1e-4, 1e1),
        ...
        for j, ph in enumerate(phase):
            ...
            if trange is not None:
                zpd = (
                    zp[field]
                    .where(
                        np.logical_and(zp["time"] >= trange[0], zp["time"] < trange[1])
                    )
                    .dropna("time")
                )
            else:
...
def plot_stacked_bar(frac, x="time", **kwargs):
    ...
    color_idx = dict(Pthm=0, Ptrb=1, dPimag=2, oPimag=3, Wsg=4, Wext=5)
    colors = {"hot": c_hot, "2p": c_2p, "whole": c_2p}
    ...
    for varph in frac.varph:
        ...
        if x == "time":
            plt.fill_between(
                frac.time, f0, f0 + frac.sel(varph=varph), label=label, color=c
            )
        elif x == "model":
            plt.bar(
                frac.model,
                frac.sel(varph=varph),
                bottom=f0,
                label=label,
                color=c,
                **kwargs
...
def plot_pressure_fraction(ptdata, ph="2p", rolling=None, normed=True):
    ...
    frac = pt.sel(variable=["Pthm", "Ptrb", "dPimag", "oPimag"], phase=[ph]).stack(
        varph=["phase", "variable"]
    )
    if normed:
        frac = frac / Ptot
    ...
    plt.xlabel("time [Myr]")
    plt.ylabel(r"$P_{\rm comp}/P_{\rm tot}$")
    if normed:
        plt.ylim(0, 1)
...
def plot_weight_fraction(wtdata, rolling=None, normed=True):
    ...
    frac = wt.sel(variable=["Wsg", "Wext"], phase=["2p"]).stack(
        varph=["phase", "variable"]
    )
    if normed:
        frac = frac / Wtot
...
def plt_pressure_weight_tevol(sim, rolling=None, normed=True):
    ...
    zmin = max(10, sim.domain["dx"][2])
    zslice = slice(-zmin, zmin)
    area = pw["A"].sel(z=zslice).mean(dim="z")
    area_frac = area / area.sel(phase="whole")

    wtevol = pw.to_array().sel(variable=["W", "Wsg", "Wext"], z=zslice).mean(dim="z")
    wtevol = wtevol  # /area_frac

    da_p = pw.to_array().sel(
        variable=["Ptot", "Ptrb", "Pthm", "Pimag", "dPimag", "oPimag"]
    )
    pmid_tevol = da_p.sel(z=zslice).mean(dim="z")
    pzmax_tevol = da_p.sel(z=(-1000, 1000), method="nearest").mean(dim="z")
    ptevol = pmid_tevol - pzmax_tevol
    ptevol = ptevol / area_frac

    fig, axes = plt.subplots(
        3, 1, figsize=(10, 10), gridspec_kw=dict(wspace=1), sharex=True
    )
    ...
    plt.plot(
        ptevol.time,
        ptevol.sel(variable="Ptot", phase="2p"),
        label=r"$\Delta P_{\rm tot}$",
    )
    ...
    # pressure fraction
    plt.sca(axes[1])
    plot_pressure_fraction(ptevol, ph="whole", rolling=rolling, normed=normed)
    plt.xlabel("")
...
def calc_phase_fraction(zpdata, x="time", var="fmass", zcut="H", z0=0, rolling=None):
    ...
    if (var == "fmass") & ("MOL" in phs) & ("dH2" in zpdata.coords["variable"]):
        ...
        if (1 - total).std() > 0.01:
            print(
                "Warning: phase fraction doesn't sum up to 1 -- sum={}".format(
                    total.mean()
                )
            )
...
def plot_phase_fraction(frac, x="time", zcut="H", z0=0, rolling=None):
    ...
    for c, ph in zip(colors, frac.phase):
        if x == "time":
            plt.fill_between(
                frac.time, f0, f0 + frac.sel(phase=ph), label=ph.data, color=c
            )
        elif x == "model":
...
def plt_upsilon_comparison(sa, trange=slice(300, 500), torb=None, norm=False):
    ...
    for m in sa.models:
        ...
        Pstack[m] = (
            ptevol.sel(time=trange, phase=["2p"])
        ...
        wtevol = pw.to_array().sel(variable=["Wsg", "Wext"], z=zslice).mean(dim="z")
        Wstack[m] = (
            wtevol.sel(time=trange, phase=["2p"])
            .stack(varph=["phase", "variable"])
            .mean(dim="time")
        )
...
def plt_phase_comparison(
    sa, thermal_only=False, trange=slice(300, 500), torb=None, **zkwargs
):
