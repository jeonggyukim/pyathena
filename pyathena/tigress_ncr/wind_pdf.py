...
import xarray as xr
import os
def calc_pdf(dset_z,dbin=0.02,dVol=1.,
    ...
    for wf in fields:
        ...
        key='{}-{}-{}'.format(xf,yf,wf)
        if wf == 'volume':
            pdfout[key]=np.histogram2d(np.log10(x.data.flat),np.log10(y.data.flat),
            ...
            w=dset_z[wf].load()
            pdfout[key]=np.histogram2d(np.log10(x.data.flat),np.log10(y.data.flat),
                                        bins=[bins[xf],bins[yf]],weights=w.data.flatten())
            normout[wf] = w.where(x>0).sum().data
        xf='vin'
        x=-dset_z['vout'].load()
        y=dset_z[yf].load()
        if wf == 'volume':
            pdfin[key]=np.histogram2d(np.log10(x.data.flat),np.log10(y.data.flat),
                                        bins=[bins[xf],bins[yf]])
...
def create_wind_pdf(dset,sim):
    ...
    extra_fields=['momflux_kin','momflux_th','momflux_mag',
                  'energyflux_kin_z','energyflux_th','energyflux_kin',
                  'poyntingflux']
    fields = default_fields+extra_fields
    for z in [500,1000]:
        print(z)
        dset_z=dset.sel(z=[z,-z])
        pdfout,pdfin,normout,normin = calc_pdf(dset_z,dbin=dbin,dVol=dVol,fields=fields)

        pdf_ds=xr.Dataset()
        for key in pdfout:
            xf,yf,wf = key.split('-')
            pdf_ds[wf]=xr.DataArray(pdfout[key][0].T/normout[wf]/dbin**2,
        ...
        pdf_ds.to_netcdf('{}/vz_pdf/{}.pdf-out.{:d}.nc'.format(sim.basedir,sim.basename,z))
        pdf_ds.close()

        pdf_ds=xr.Dataset()
        for key in pdfin:
            xf,yf,wf = key.split('-')
            pdf_ds[wf]=xr.DataArray(pdfin[key][0].T/normin[wf]/dbin**2,
        ...
        pdf_ds.attrs['sfr']=dset.attrs['sfr']
        pdf_ds.attrs['NxNyNt']=Nx*Ny*Nt
        pdf_ds.to_netcdf('{}/vz_pdf/{}.pdf-in.{:d}.nc'.format(sim.basedir,sim.basename,z))
        pdf_ds.close()
