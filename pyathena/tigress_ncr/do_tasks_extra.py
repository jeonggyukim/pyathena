...
extrema.update(dict(vzout=(0.1, 2.0e3)))
extrema.update(dict(vzin=(0.1, 2.0e3)))
extrema.update(dict(sound_speed=(0.1, 2.0e3)))
extrema.update({("gas", "temperature"): (1, 1.0e9)})
extrema.update(dict(CII_luminosity=(1.0e20, 1.0e40)))
extrema.update({"xray_luminosity_0.5_7.0_keV": (1.0e20, 1.0e40)})
extrema.update({"H_alpha_luminosity": (1.0e20, 1.0e40)})
extrema.update({"HI_21cm_luminosity": (1.0e15, 1.0e35)})
...
def get_inout_pdfs(box):
    out_profile = yt.create_profile(
        ...
        extrema=dict(
            vzout=(0.1, 2.0e3), sound_speed=(0.1, 2.0e3)
        ),  # , temperature=(10,1.e9)),
    )
    ...
    in_profile = yt.create_profile(
        ...
        extrema=dict(
            vzin=(0.1, 2.0e3), sound_speed=(0.1, 2.0e3)
        ),  # temperature=(10,1.e9)),
    )
...
def do_pdf(s, num):
    ...
    for lf in lumfields:
        ...
        ppvlist = []
        for z1, z2 in zip(
            np.arange(zmin, zmax, dz), np.arange(zmin + dz, zmax + dz, dz)
        ):
            print(z1, z2)
            # pdfs
            box = ds.r[:, :, z1:z2]
