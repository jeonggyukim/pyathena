...
# args (`--T-min`, `--T-max`, `--N-T`) override the tier defaults
...
def write_ascii(out_path, label, data, T_min, T_max, tier='hot'):
...
def main(argv=None):
    ...
    # Override the tier defaults if any CLI grid args differ from
    # the global default; otherwise use per-ion tier.
    cli_override = (
        args.T_min != DEFAULT_T_MIN
        or args.T_max != DEFAULT_T_MAX
        or args.N_T != DEFAULT_N_T
    )
    if cli_override:
        T_grid_uniform = np.logspace(
            np.log10(args.T_min), np.log10(args.T_max), args.N_T)
        print(f"Temperature grid (CLI override applied to all ions): "
              f"{args.N_T} points, {args.T_min:.3g} -> {args.T_max:.3g} K")
    else:
        T_grid_uniform = None
        for tier, (tmin, tmax, nT) in TIER_GRIDS.items():
    ...
    for label in ions:
        ...
        if T_grid_uniform is not None:
            T_grid = T_grid_uniform
            tier = 'cli'
        else:
            tier = TIER_BY_LABEL.get(label, DEFAULT_TIER)
            tmin, tmax, nT = TIER_GRIDS[tier]
        ...
        tmin_w = float(T_grid[0])
        tmax_w = float(T_grid[-1])
        write_ascii(out_path, label, data, tmin_w, tmax_w, tier=tier)
        nz_A = (data['A'] > 0).sum()
