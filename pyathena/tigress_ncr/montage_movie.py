import pyathena as pa
import matplotlib.pyplot as plt
from pyathena.plt_tools.make_movie import make_movie
import glob, os, sys
from shutil import copyfile
import os.path as osp
import argparse

def get_time_from_zprof(s, inum):
    zpf = glob.glob(os.path.join(s.basedir, "zprof/", f"*.{inum:04d}.whole.zprof"))
    with open(zpf[0], "r") as fp:
        l = fp.readline()
        time = float(l[l.rfind("=") + 1 :])
    return time


def stitch_number_densities(s, i, tf="layer"):
    fields = [
        "H_nuclei_density",
        "H_p0_number_density",
        "H_p1_number_density",
        "H2_number_density",
    ]
    field_names = ["total", "atomic (HI)", "ionized (HII)", "molecular (H$_2$)"]

    with plt.style.context("dark_background", {"figure.dpi": 200}):
        fig, axes = plt.subplots(
            1,
            4,
            figsize=(10, 7),
            gridspec_kw=dict(wspace=0, hspace=0, width_ratios=[0.65, 0.3, 0.3, 0.3]),
        )

        for field, name, ax in zip(fields, field_names, axes.flatten()):
            if tf == "linramp":
                head = f"{field}_linramp"
            else:
                head = field
            files = sorted(
                glob.glob(os.path.join(s.basedir, "volume", f"{head}_time_????.png"))
            )
            inum = int(files[i].split("_")[-1].split(".")[0])
            time = get_time_from_zprof(s, inum)
            plt.sca(ax)
            img = plt.imread(files[i])
            Nx, Ny, Nch = img.shape
            plt.imshow(img)
            plt.annotate(
                name,
                (0.5, 0.0),
                ha="center",
                va="top",
                xycoords="axes fraction",
                weight="bold",
            )
            if name == "total":
                plt.xlim(Nx * 0.35, Nx)
            else:
                plt.xlim(Nx * 0.35, Nx * 0.65)
            plt.axis("off")
        axes[0].annotate(
            f"t={time*s.u.Myr:5.1f}Myr",
            (0.1, 1.0),
            va="top",
            ha="left",
            xycoords="axes fraction",
            weight="bold",
        )

        fig.savefig(
            os.path.join(
                s.basedir, "volume", f"number_density_{tf}_time_{inum:04d}.png"
            ),
            bbox_inches="tight",
        )


def stitch_observables(s, i, tf="layer"):
    fields = ["CII_luminosity", "H_alpha_luminosity", "xray_luminosity_0.5_7.0_keV"]
    field_names = ["CII", r"${\bf H_\alpha}$", "X-ray (0.5-7.0 keV)"]

    with plt.style.context("dark_background", {"figure.dpi": 200}):
        fig, axes = plt.subplots(
            1, 3, figsize=(10, 7), gridspec_kw=dict(wspace=0, hspace=0)
        )

        for field, name, ax in zip(fields, field_names, axes):
            if tf == "linramp":
                head = f"{field}_linramp"
            else:
                head = field
            files = sorted(
                glob.glob(os.path.join(s.basedir, "volume", f"{head}_time_????.png"))
            )
            inum = int(files[i].split("_")[-1].split(".")[0])
            time = get_time_from_zprof(s, inum)
            plt.sca(ax)
            img = plt.imread(files[i])
            Nx, Ny, Nch = img.shape
            plt.imshow(img)
            # plt.annotate(name,(0.5,0.),ha='center',va='top',xycoords='axes fraction',weight='bold')
            plt.xlim(Nx * 0.35, Nx)
            plt.axis("off")
        axes[0].annotate(
            f"t={time*s.u.Myr:5.1f}Myr",
            (0.1, 1.0),
            va="top",
            ha="left",
            xycoords="axes fraction",
            weight="bold",
        )
        fig.savefig(
            os.path.join(s.basedir, "volume", f"luminosity_{tf}_time_{inum:04d}.png"),
            bbox_inches="tight",
        )


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "-b", "--basedir", type=str, default=None, help="Name of the basedir."
    )
    args = vars(parser.parse_args())
    locals().update(args)
    print(basedir)

    fields = [
        "H_nuclei_density",
        "H_p0_number_density",
        "H_p1_number_density",
        "H2_number_density",
        "CII_luminosity",
        "H_alpha_luminosity",
        "xray_luminosity_0.5_7.0_keV",
    ]

    s = pa.LoadSimTIGRESSNCR(basedir)

    # check files
    nfiles = None
    all_identical = True
    for f in fields:
        for tf in ['_linramp','']:
            files = sorted(
                glob.glob(
                    osp.join(s.basedir, "volume", f"{f}{tf}_time_????.png")
                )
            )
            print(f'found {len(files)} images for {f}{tf}')
            if nfiles is not None:
                if nfiles != len(files):
                    all_identical = False
            else:
                nfiles = len(files)

    if not all_identical: sys.exit()

    # montage
    for i in range(nfiles):
        print(f"montage [{i}/{nfiles}]")
        stitch_number_densities(s, i, tf="linramp")
        stitch_number_densities(s, i, tf="layer")

        stitch_observables(s, i, tf="linramp")
        stitch_observables(s, i, tf="layer")
        plt.close("all")

    # movies
    for f in fields:
        fin = os.path.join(s.basedir, "volume", f"{f}_linramp_time_????.png")
        fout = osp.join(s.basedir, f"{s.basename}_{f}_linramp_time.mp4")
        make_movie(fin, fout, fps_in=15, fps_out=15)
        fin = os.path.join(s.basedir, "volume", f"{f}_time_????.png")
        fout = osp.join(s.basedir, f"{s.basename}_{f}_time.mp4")
        make_movie(fin, fout, fps_in=15, fps_out=15)
    fields = ["number_density", "luminosity"]
    for f in fields:
        for tf in ["linramp", "layer"]:
            fin = os.path.join(s.basedir, "volume", f"{f}_{tf}_time_????.png")
            fout = osp.join(s.basedir, f"{s.basename}_{f}_{tf}_time.mp4")
            make_movie(fin, fout, fps_in=15, fps_out=15)
            copyfile(
                fout,
                f"/tigress/changgoo/public_html/temporary_movies/TIGRESS-NCR_volume/{osp.basename(fout)}",
            )
