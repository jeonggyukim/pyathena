...
class PaperIFigures:
    ...
    def get_label(tf, wf, normed=False):
        ...
        yl_u = ylabel[wf][1:-1]
        if wf in ["netcool", "net_cool_rate", "cool_rate", "heat_rate"] and normed:
            yl_u += r"/\mathcal{L}(|z|<300{\rm\,pc})"
    ...
    def plot_1Dpdf(
        ...
        if reduced == "all":
            ...
            phdef_reduced = dict(
                CNM=[0, 1], UNM=[2, 3], WNM=[4], WPIM=[5], WCIM=[6], WHIM=[7], HIM=[8]
            )
        elif reduced == "six":
            phdef_reduced = dict(
                CNM=[0, 1], UNM=[2, 3], WNM=[4], WIM=[5, 6], WHIM=[7], HIM=[8]
            )
        elif reduced == "five":
            phdef_reduced = dict(
                CNM=[0, 1], UNM=[2, 3], WNM=[4], WIM=[5, 6], HIM=[7, 8]
            )
        elif reduced == "one":
        ...
        for phname, idx in phdef_reduced.items():
            ...
            y = (
                pdf.sel(
                    phase=[ph["name"] for ph in np.array(phdef)[np.atleast_1d(idx)]]
                ).sum(dim="phase")
                / dx
                / normvalue
            )
            if wf in ["vol", "mass", "nH2"]:
                y /= ytot
            if "time" in pdf.coords:
                q1, q2, q3 = quantiles
                ...
                if verbose:
                    print(
                        phname,
                        "{:5.3f}".format((y.mean(dim="time") * dx).sum().data),
                        end=" ",
                    )
                if phname == "UIM":
                    continue
        ...
        ymins = dict(
            ...
            nH2=1.0e-4,
            netcool=1.0e-22,
            net_cool_rate=1.0e-22,
            cool_rate=1.0e-22,
            heat_rate=1.0e-22,
        )
        xranges = dict(
            T=(10, 2.0e7), T1=(10, 1.0e8), nH=(1.0e-5, 1.0e3), pok=(10.0, 1.0e7)
        )
        plt.xscale("log")
        plt.yscale("log")
        if xlabel:
    ...
    def get_tseries(pdf, phdef, reduced="all"):
        if reduced == "all":
            ...
            phdef_reduced = dict(
                Cold=[0, 1], UNM=[2, 3], WNM=[4], WIM=[5, 6], Hot=[7, 8]
            )
        elif reduced == "one":
            phdef_reduced = dict(all=np.arange(9))
    ...
    def print_pdf_table(sa, ms, tr, reduced="seven", kind="percentile"):
        ...
        for wf in ["mass", "vol"]:
            ...
            for i, (m, ts) in enumerate(zip(ms, tr)):
                s = sa.set_model(m)
                data, colors, labels = PaperIFigures.get_tseries(
                    getattr(s.pdf, tf)[wf].sel(time=ts),
                    s.pdf.phdef,
    ...
    def plot_box_comparison(sa, ms, ts, wf, mcolors, reduced=None):
        ...
        for i, m in enumerate(ms):
            s = sa.set_model(m)
            data, colors, labels = PaperIFigures.get_tseries(
                getattr(s.pdf, tf)[wf].sel(time=ts[i]),
                s.pdf.phdef,
                reduced=wf if reduced is None else reduced,
            )
            box = plt.boxplot(
                data,
                positions=np.arange(len(data)) + offset[i],
                widths=w,
                whis=[16, 84],
                showfliers=False,
                patch_artist=True,
        ...
        for d, c, label, xc in zip(data, colors, labels, np.arange(len(data))):
    ...
    @staticmethod
    def add_box_legend(
        ms, mcolors, x0=0.90, dx=0.015, y0=0.95, dy=0.12, fontsize=16, linewidth=3
    ):
        for i, m in enumerate(ms):
            c = mcolors[m]
            plt.annotate(
            ...
            plt.annotate(
                ...
                ha="center",
                va="top",
                **texteffect(fontsize, linewidth=linewidth),
            )
    ...
    def plot_box_comparison_both(sa, ms, ts, reduced=None):
        ...
        plt.sca(axes[0])
        PaperIFigures.plot_box_comparison(
            sa, ms, ts, "mass", reduced=reduced, legend=None
        )
        plt.ylabel("Mass Fraction")

        plt.sca(axes[1])
        PaperIFigures.plot_box_comparison(sa, ms, ts, "vol", reduced=reduced)
    ...
    def plot_tseries(sa, ms, tr, wf="mass"):
        ...
        fig, axes = plt.subplots(
            nmodel, 2, figsize=(10, 3 * nmodel), sharey="row", sharex="col"
        )
        for i, m in enumerate(ms):
            s = sa.set_model(m)
            tf = "nH"
            data, colors, labels = PaperIFigures.get_tseries(
                getattr(s.pdf, tf)[wf].sel(time=tr), s.pdf.phdef, reduced="seven"
            )
            plt.sca(axes[i, 0])
            for d, c, label in zip(data, colors, labels):
                plt.plot(d["time"], d, color=c, label=label)
            plt.ylabel("{} fraction".format(wf))
            plt.sca(axes[i, 1])
            box = plt.boxplot(
                data, patch_artist=True, medianprops=dict(color="k"), labels=labels
            )
            for p, c in zip(box["boxes"], colors):
                p.set_facecolor(c)
            plt.annotate(
                m, (0.95, 0.95), xycoords="axes fraction", ha="right", va="top"
            )
        plt.sca(axes[-1, 0])
        plt.xlabel("time")
        plt.tight_layout()
...
def print_latex_string(data, labels, m, kind="percentile"):
    ...
    for d, label in zip(data, labels):
        ...
        if q[1] < 0.1:
            latex_string2 += " & {:4.2f}^{{+{:4.2f}}}_{{-{:4.2f}}}".format(
                q[1], q[2] - q[1], q[1] - q[0]
            )
        else:
            latex_string2 += " & {:3.1f}^{{+{:3.1f}}}_{{-{:3.1f}}}".format(
                q[1], q[2] - q[1], q[1] - q[0]
            )
