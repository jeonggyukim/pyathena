...
def CII_rec_rate(T):
    """New CII recombination rate. Added by c268ea0f96bfb7212312ea260ebe8b4036a8d76f
    Re-implemented just for self-consistency. One can turn on this feature with
    <cooling>/iCII_rec_rate = 1. Default is 0 (approximate recombination
    coefficient). Since the approximation expression is very close to the accurate
    one for T< 10^4K, nothing should change essentially.
    """
    A = 2.995e-9
    B = 0.7849
    T0 = 6.670e-3
    ...
    term2 = np.sqrt(T / T1)
    alpharr = A / (
        term1 * np.power(1.0 + term1, 1.0 - BN) * np.power(1.0 + term2, 1.0 + BN)
    )
    alphadr = np.power(T, -3.0 / 2.0) * (
        6.346e-9 * np.exp(-1.217e1 / T)
        + 9.793e-09 * np.exp(-7.38e1 / T)
        + 1.634e-06 * np.exp(-1.523e04 / T)
    )
...
def get_xCII(
    ...
    lnT = np.log(T)
    if iCII_rec_rate:
        # added per c268ea0f96bfb7212312ea260ebe8b4036a8d76f
        k_Cplus_e = CII_rec_rate(T)
    else:
        k_Cplus_e = np.where(
            T < 10.0, 9.982641225129824e-11, np.exp(-0.7529152 * lnT - 21.293937)
        )
        ...
        k_Cplus_gr = (
            ...
            * cCp_[0]
            / (
                1.0
                + cCp_[1]
                * np.power(psi_gr, cCp_[2])
                * (
                    1.0
                    + cCp_[3]
                    * np.power(T, cCp_[4])
                    * np.power(psi_gr, -cCp_[5] - cCp_[6] * lnT)
                )
            )
            * Z_d
        )
...
def heatCR(nH, xe, xHI, xH2, xi_CR, old=False):
    ...
    qH2 = (
        np.where(log_nH < 2.0, 10.0 * eV_cgs, 0.0)
        + np.where(
            np.logical_and(log_nH >= 2.0, log_nH < 4.0),
            (10 + 3 * (log_nH - 2.0) * 0.5) * eV_cgs,
            0.0,
        )
        + np.where(
            np.logical_and(log_nH >= 4.0, log_nH < 7.0),
            (13 + 4 * (log_nH - 4.0) / 3) * eV_cgs,
            0.0,
        )
        + np.where(
            np.logical_and(log_nH >= 7.0, log_nH < 10.0),
            (17 + (log_nH - 7.0) / 3) * eV_cgs,
            0.0,
        )
        + np.where(log_nH >= 10.0, 18.0 * eV_cgs, 0.0)
    )

    qtot = xHI * qHI + 2 * xH2 * qH2
    if old:
        return ktot * qtot
    else:
        return xi_CR * qtot
...
def heatH2(
    ...
    if ikgr_H2 == 0:  # Constant coeff
        ...
        kgr = (
            ...
            * np.sqrt(T2)
            * 2.0
            / (1 + 0.4 * np.sqrt(T2) + 0.2 * T2 + 0.08 * T2 * T2)
        )
    ...
    else:  # both MH79 and DB96
        de = 1.6 * xHI * np.exp(-((400.0 / T) ** 2)) + 1.4 * xH2 * np.exp(
            -12000.0 / (1200.0 + T)
        )
...
def q10CII_(nH, T, xe, xHI, xH2):
    ...
    tmp = np.power(T, 0.07)
    k10oH2 = np.where(T < 500.0, (5.33 + 0.11 * T2) * 1.0e-10, 3.74757785025e-10 * tmp)
    k10pH2 = np.where(T < 500.0, (4.43 + 0.33 * T2) * 1.0e-10, 3.88997286356e-10 * tmp)
...
def coolHI(nH, T, xHI, xe):
    ...
    LambdaLyB = (
        fac * exfacLyB * (upsilon3s + upsilon3p + upsilon3d) * xHI * xe * nH * kB * TLyB
    )
...
def coolH2G17(nH, T, xHI, xH2, xHII, xe, xHe=0.1):
    ...
    Tmax_H2 = 6000.0  # maximum temperature above which use Tmax
    Tmin_H2 = 10.0  # minimum temperature below which cut off cooling
    ...
    LHI += np.where(
        np.logical_and(T >= 100.0, T < 1000.0),
        np.power(
            10,
            -2.4311209e1
            + 3.5692468e0 * logT3
    ...
    LH2 = np.power(
        ...
        -2.3962112e1
        + 2.09433740e0 * logT3
        - 0.77151436e0 * logT3_2
        + 0.43693353e0 * logT3_3
        - 0.14913216e0 * logT3_4
        - 0.033638326e0 * logT3_5,
    )
    # He
    LHe = np.power(
        10,
        -2.3689237e1
        + 2.1892372e0 * logT3
        - 0.81520438e0 * logT3_2
        + 0.29036281e0 * logT3_3
        - 0.16596184e0 * logT3_4
        + 0.19191375e0 * logT3_5,
    )
    # H+
    LHplus = np.power(
        10,
        -2.1716699e1
        + 1.3865783e0 * logT3
        - 0.37915285e0 * logT3_2
        + 0.11453688e0 * logT3_3
        - 0.23214154e0 * logT3_4
        + 0.058538864e0 * logT3_5,
    )
    # e
    Le = np.where(
        T < 200.0,
        ...
        np.power(
            ...
            + 1.5728955e0 * logT3
            - 0.213351e0 * logT3_2
            + 0.96149759e0 * logT3_3
            - 0.91023195e0 * logT3_4
            + 0.13749749e0 * logT3_5,
        ),
    )
    ...
    T3 = T * 1.0e-3
    Gamma_LTE_HR = (9.5e-22 * np.power(T3, 3.76)) / (
        1.0 + 0.12 * np.power(T3, 2.1)
    ) * np.exp(-np.power(0.13 / T3, 3)) + 3.0e-24 * np.exp(-0.51 / T3)
    Gamma_LTE_HV = 6.7e-19 * np.exp(-5.86 / T3) + 1.6e-18 * np.exp(-11.7 / T3)
    Gamma_LTE = Gamma_LTE_HR + Gamma_LTE_HV
    # Total cooling rate
    Gamma_tot = np.where(
        Gamma_n0 > 1e-100, Gamma_LTE / (1.0 + Gamma_LTE / Gamma_n0), 0.0
    )
...
def coolH2rovib(nH, T, xHI, xH2):
    ...
    f3 = 2.4e-24 * sqrtT3 * T3 * np.exp(-2.0 * T3inv) * (x3 / (1.0 + x3 / n3))
    f4 = (
        1.7e-23
        * sqrtT3
        * T3
...
def cool3Level_(q01, q10, q02, q20, q12, q21, A10, A20, A21, E10, E20, E21, xs):
...
def coolCO(nH, T, xe, xHI, xH2, xCO, dvdr):
    ...
    L0CO = interpolate.interp1d(
        TCO_, L0CO_, kind="linear", bounds_error=False, fill_value="extrapolate"
    )
    ...
    facT = np.power(1.0 - np.exp(-T1), 1.0e3)  # Shuts off cooling if T < ~10 K
    ...
    inv_LCO = (
        1.0 / L0
        + neff / LLTE
        + 1.0 / L0 * np.power(neff / nhalf, alpha) * (1.0 - nhalf * L0 / LLTE)
    )

    return (1.0 / inv_LCO) * neff * xCO * facT


def fshld_H2(NH2, b5=3.0):
    x = 2.0e-15 * NH2
    sqrt_onepx = np.sqrt(1.0 + x)
    return 0.965 / (1.0 + x / b5) ** 2 + 0.034970262640168365 / sqrt_onepx * np.exp(
        -8.5e-4 * sqrt_onepx
    )
                        ...
                        + (
                            ...
                            + (
                                ...
                                + (
                                    ...
                                    + (
                                        -2.63197617e-3
                                        + (1.11954395e-4 + (-2.03914985e-6) * lnTe)
                                        * lnTe
                                    )
                                    * lnTe
                                )
    ...
    alpha_gr = (
        ...
        * cHp_[0]
        / (
            1.0
            + cHp_[1]
            * np.power(psi_gr, cHp_[2])
            * (
                1.0
                + cHp_[3]
                * np.power(T, cHp_[4])
                * np.power(psi_gr, -cHp_[5] - cHp_[6] * lnT)
            )
        )
        * Z_d
    )
...
def coeff_coll_H2(nH, T, xHI, xH2):
    ...
    k9l_ = 6.67e-12 * np.sqrt(T) * np.exp(-(1.0 + 63590.0 * Tinv))
    ...
    n2ncr = nH * ncrinv
    k_H2_HI = np.power(
        10, np.log10(k9h_) * n2ncr / (1.0 + n2ncr) + np.log10(k9l_) / (1.0 + n2ncr)
    )
    k_H2_H2 = np.power(
        10, np.log10(k10h_) * n2ncr / (1.0 + n2ncr) + np.log10(k10l_) / (1.0 + n2ncr)
    )
