...
for m in models:
    ...
    for num in nums:
        ...
        if os.path.isfile(os.path.join(savdir, coolpdfname)):
            continue
        ...
        data["soft-Xray"] = xr.DataArray(
            em * data["nH"] ** 2, coords=[data.z, data.y, data.x]
        )
        emax = 7.0  # keV
        em = get_xray_emissivity(data["T"].data, 1.0, emin, emax, energy=True)
        data["full-Xray"] = xr.DataArray(
            em * data["nH"] ** 2, coords=[data.z, data.y, data.x]
        )
        ...
        S_X = xr.Dataset()
        S_X["soft_yz"] = (
            data["soft-Xray"].sum(dim="x") * s.domain["dx"][0] * s.u.cm / 4 / np.pi
        )
        S_X["soft_xz"] = (
            data["soft-Xray"].sum(dim="y") * s.domain["dx"][1] * s.u.cm / 4 / np.pi
        )
        S_X["soft_xy"] = (
            data["soft-Xray"].sum(dim="z") * s.domain["dx"][2] * s.u.cm / 4 / np.pi
        )
        S_X["full_yz"] = (
            data["full-Xray"].sum(dim="x") * s.domain["dx"][0] * s.u.cm / 4 / np.pi
        )
        S_X["full_xz"] = (
            data["full-Xray"].sum(dim="y") * s.domain["dx"][1] * s.u.cm / 4 / np.pi
        )
        S_X["full_xy"] = (
            data["full-Xray"].sum(dim="z") * s.domain["dx"][2] * s.u.cm / 4 / np.pi
        )
