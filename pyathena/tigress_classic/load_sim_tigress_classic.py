...
class LoadSimTIGRESS(LoadSim, SliceProj, Zprof):
    """LoadSim class for analyzing TIGRESS-Classic simulations.
    """

    def __init__(self, basedir, savdir=None, load_method='xarray',
                 muH = 1.4271, verbose=False):
        """The constructor for LoadSimTIGRESS class
        ...
        super(LoadSimTIGRESS,self).__init__(basedir, savdir=savdir,
...
class LoadSimTIGRESSAll(object):
    ...
    def __init__(self, models=None, muH=None):
        ...
        for mdl, basedir in models.items():
            if not osp.exists(basedir):
                print('[LoadSimTIGRESSAll]: Model {0:s} doesn\'t exist: {1:s}'.format(
                    mdl,basedir))
            else:
                ...
                if mdl in muH:
                    ...
                    print('[LoadSimTIGRESSAll]: muH for {0:s} has to be set'.format(
                          mdl))
    ...
    def set_model(self, model, savdir=None, load_method='pyathena', verbose=False):
        ...
        try:
            ...
            self.sim = LoadSimTIGRESS(self.basedirs[model], savdir=savdir,
                                         muH=self.muH[model],
                                         load_method=load_method, verbose=verbose)
