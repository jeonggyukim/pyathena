...
def set_my_dfi(dfi):
    ...
    my_dfi['chi_FUV']['norm'] = LogNorm(1.e-1,1.e2)
    my_dfi['Erad_LyC']['cmap'] = cmr.sunburst
    my_dfi['Erad_LyC']['norm'] = LogNorm(1.e-16,1.e-12)
...
def paper_snapshot(s,num,z0=0,dz=15,dt=10,outdir=None,
                     scratch_dir='/scratch/gpfs/changgoo/TIGRESS-NCR/'):
    fields = ['nH','T','xHI','xe','xHII','xH2','chi_FUV',
              'cool_rate','net_cool_rate','Erad_LyC']
    ds = s.load_vtk(num)
    ...
    gs0=gridspec.GridSpec(2,1,
                         left=0,right=0.25,bottom=0.00,top=1,
                         height_ratios=[1,1],
                         wspace=0,hspace=0)
    ...
    for i,f in enumerate(['Sigma_gas','EM']):
        ...
        im=plt.imshow(prj['z'][f],cmap=dfi[f]['cmap'],norm=dfi[f]['norm'],
                     extent=np.array([ds.domain['le'][0],ds.domain['re'][0],
                                      ds.domain['le'][1],ds.domain['re'][1]])/1.e3)
        # plt.ylim(-2,2)
        # cbar=plt.colorbar(im,cax=cax,orientation='horizontal')
        if i == 0:
          cbar=plt.colorbar(im,pad=0.02,shrink=0.8,location='top',
                            label=dfi[f]['cbar_kwargs']['label'])
        else:
          cbar=plt.colorbar(im,pad=0.02,shrink=0.8,location='bottom',
                            label=dfi[f]['cbar_kwargs']['label'])
    ...
    ax=axes['Sigma_gas']
    scatter_sp(sp,ax,'z',kpc=True,norm_factor=50,u=s.u,agemax=40,kind='slc',dist_max=50)
    legend_sp(ax,norm_factor=50,mass=[1.e3,1.e4],location='right')
    ...
    gs1=gridspec.GridSpec(2,3,
                         left=0.34,right=1,bottom=0.,top=1.0,
                         wspace=0.,hspace=0.0)
...
#     scatter_sp(sp,ax,'z',kpc=True,norm_factor=200,u=s.u,agemax=40)
    ...
    #     slc.plot(**dfi[f])
        if f == 'net_cool_rate':
            im=plt.imshow(slc,cmap=dfi['cool_rate']['cmap'],norm=dfi['cool_rate']['norm'],extent=extent)
            im=plt.imshow(-slc,cmap=dfi['heat_rate']['cmap'],norm=dfi['heat_rate']['norm'],extent=extent)
        else:
            im=plt.imshow(slc,cmap=dfi[f]['cmap'],norm=dfi[f]['norm'],extent=extent)

        ax.set_aspect('equal')
        ax.set_title('')
        decorate_ax(dfi[f]['cbar_kwargs']['label'],ax,iscbar=False)
        if i//3 == 0: cbar=plt.colorbar(im,pad=0.02,shrink=0.8,location='top',label=dfi[f]['cbar_kwargs']['label'])
        elif i//3 == 1: cbar=plt.colorbar(im,pad=0.02,shrink=0.8,location='bottom',label=dfi[f]['cbar_kwargs']['label'])
        if f == 'T': cbar.set_ticks([100,1.e4,1.e6])
        if f in ['xHI','chi_FUV']:
        # if f != 'Erad_LyC':
            plt.contour(slc.x/1.e3,slc.y/1.e3,LyC_slc,levels=[1.e-15,1.e-14,1.e-13,1.e-12],
                        linewidths=2,
    ...
    #         plt.contour(slc.x/1.e3,slc.y/1.e3,nH_slc,levels=[1.,1.e2],cmap=dfi['density']['cmap'])
        ...
        if f in ['nH']:
            ...
            plt.scatter(snsel['x1']/1.e3,snsel['x2']/1.e3,s=100,linewidths=0,
